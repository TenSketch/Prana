...
export class MyBookingsComponent {
  ...
  sendCancelWhatsApp(item: any): void {
    const phoneNumber = '919840167444';
    const message = `Booking ID: ${item.booking_id}\n\nI want to cancel my reservation in ${item.rooms.restort}`;
    window.location.href = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(
      message
    )}`;
  }
